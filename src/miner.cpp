...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        LogPrint("tecsacoin", "CreateNewBlock(): total size %u\n", nBlockSize);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("tecsacoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("tecsacoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("tecsacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tecsacoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrint("tecsacoin", "Running tecsacoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
