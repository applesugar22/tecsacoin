...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::tecsacoin);
    ...
    if (!settings.contains("nAnonymizetecsacoinAmount"))
        settings.setValue("nAnonymizetecsacoinAmount", 1000);

    nAnonymizetecsacoinAmount = settings.value("nAnonymizetecsacoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizetecsacoinAmount"))
        SoftSetArg("-anonymizetecsacoinamount", settings.value("nAnonymizetecsacoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in tecsacoin.cpp during shotdown to also remove the window positions
...
//            return settings.value("fShowMasternodesTab");
        ...
        case AnonymizetecsacoinAmount:
            return QVariant(nAnonymizetecsacoinAmount);
        case Listen:
...
//            break;
        ...
        case AnonymizetecsacoinAmount:
            nAnonymizetecsacoinAmount = value.toInt();
            settings.setValue("nAnonymizetecsacoinAmount", nAnonymizetecsacoinAmount);
            emit anonymizetecsacoinAmountChanged(nAnonymizetecsacoinAmount);
            break;
        case CoinControlFeatures:
